...
import { listGuests, deleteGuest as deleteGuestSvc } from '../services/guests';
import type { Guest } from '../types';
import { StatusBadge } from '../components/StatusBadge';
import ExportButtons from '../components/ExportButtons';
import ConfirmModal from '../components/ConfirmModal';
import { Search, Trash2, Loader2, Users, X } from 'lucide-react';
import { calculateAge } from '../utils/cpf';
import { matchGuest } from '../utils/search';
import { toast } from 'sonner';

export default function GuestDeleteListPage() {
  ...
  const executeDeleteGuest = async () => {
    ...
    try {
      ...
      toast.success(`Hóspede "${guestToDelete.name}" e seus registros foram excluídos.`);
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold tracking-tight text-slate-900 dark:text-white flex items-center gap-2.5">
            ...
            Excluir Registros de Hóspedes
          </h1>
          <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">
            Localize e remova registros cadastrais e históricos do sistema com confirmação segura
          </p>
        </div>
      ...
      <div className="card overflow-hidden shadow-card">
        {loading ? (
          <div className="py-16 flex flex-col items-center justify-center gap-3 text-slate-500">
            ...
            <p className="text-sm font-medium">Carregando hóspedes...</p>
          </div>
        ) : filtered.length === 0 ? (
                  ...
                  >
                    <td className="py-3.5 px-5 font-bold text-slate-900 dark:text-white">
                      <div className="flex items-center gap-3">
                        <div className="h-8 w-8 rounded-xl bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 flex items-center justify-center font-bold text-xs shrink-0 group-hover:bg-rose-50 group-hover:text-rose-600 transition-colors">
                    ...
                    <td className="py-3.5 px-5 text-right">
                      <button
                        ...
                        className="btn-danger py-1.5 px-3.5 text-xs font-semibold rounded-lg inline-flex items-center gap-1.5 shadow-xs cursor-pointer"
                      >
