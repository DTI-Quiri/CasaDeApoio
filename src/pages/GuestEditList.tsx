...
import { Search, Edit3, ArrowRight, Loader2, Users, X } from 'lucide-react';
import { calculateAge } from '../utils/cpf';
import { matchGuest } from '../utils/search';

export default function GuestEditListPage() {
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold tracking-tight text-slate-900 dark:text-white flex items-center gap-2.5">
            ...
            Editar Registros de Hóspedes
          </h1>
          <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">
            Localize e selecione o hóspede para atualizar informações cadastrais, acompanhantes ou dados de atendimento
          </p>
        </div>
                  ...
                  >
                    <td className="py-3.5 px-5 font-bold text-slate-900 dark:text-white">
                      <div className="flex items-center gap-3">
                        <div className="h-8 w-8 rounded-xl bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 flex items-center justify-center font-bold text-xs shrink-0 group-hover:bg-amber-50 group-hover:text-amber-600 transition-colors">
                    ...
                    <td className="py-3.5 px-5 text-right">
                      <Link
                        className="btn-secondary py-1.5 px-3.5 text-xs font-semibold rounded-lg inline-flex items-center gap-1.5 hover:border-amber-400 hover:text-amber-600 dark:hover:text-amber-400 transition-all shadow-xs"
                        to={`/hospedes/${g.id}`}
                      >
